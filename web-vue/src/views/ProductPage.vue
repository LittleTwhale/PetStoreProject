...
const createForm = reactive({
  name: '', product_type: 'goods', price: 0, cost_price: null as number | null,
  stock: 0, pet_id: null as number | null, inventory_item_id: null as number | null,
...
const resetCreateForm = () => {
  createForm.name = ''; createForm.product_type = 'goods'
  createForm.price = 0; createForm.cost_price = null
  createForm.stock = 0; createForm.pet_id = null
  createForm.inventory_item_id = null
  createForm.description = ''; createForm.cover_image = ''
}
...
const handleCreate = async () => {
  ...
  try {
    await productApi.create({
      name: createForm.name, product_type: createForm.product_type,
      store_id: storeStore.currentStoreId, price: createForm.price,
      cost_price: createForm.cost_price ?? undefined,
      stock: createForm.stock, pet_id: createForm.pet_id ?? undefined,
...
const openEditDialog = (item: Product) => {
  ...
  editForm.name = item.name; editForm.product_type = item.product_type
  editForm.price = item.price; editForm.cost_price = item.cost_price
  editForm.stock = item.stock; editForm.pet_id = item.pet_id
...
const handleEdit = async () => {
  ...
  try {
    await productApi.update(editingId.value, {
      name: editForm.name || undefined, product_type: editForm.product_type,
      price: editForm.price, cost_price: editForm.cost_price ?? undefined,
      stock: editForm.stock, pet_id: editForm.pet_id,
  ...
  <div class="page-root">
    ...
    <div class="stats-row">
      <div class="stat-card">
        ...
        <div class="stat-body"><span class="stat-num">{{ totalCount }}</span><span class="stat-label">商品总数</span></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon" style="background:#f0f9eb;color:#67c23a"><el-icon :size="22"><Goods /></el-icon></div>
        <div class="stat-body"><span class="stat-num">{{ goodsCount }}</span><span class="stat-label">用品类</span></div>
      </div>
      <div class="stat-card">
        <div class="stat-icon" style="background:#fdf6ec;color:#e6a23c"><el-icon :size="22"><Present /></el-icon></div>
        <div class="stat-body"><span class="stat-num">{{ petCount }}</span><span class="stat-label">活体宠物</span></div>
      </div>
    </div>
    ...
    <div class="toolbar">
      <div class="filter-group">
        <el-input v-model="searchText" placeholder="搜索商品名称..." :prefix-icon="Search" clearable class="search-input" @change="fetchProducts" />
        <el-select v-model="filterType" placeholder="类型筛选" clearable class="filter-select" @change="fetchProducts">
    ...
    <div class="table-card">
      <el-table :data="products" v-loading="isLoading" stripe style="width:100%">
        ...
        <el-table-column label="类型" width="90">
          <template #default="{ row }">
            <el-tag :type="typeTag(row.product_type)" size="small">{{ typeLabel(row.product_type) }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="成本价" width="100">
          <template #default="{ row }">{{ row.cost_price != null ? `¥${Number(row.cost_price).toFixed(2)}` : '—' }}</template>
        </el-table-column>
    ...
    <!-- 创建弹窗 -->
    <el-dialog v-model="createDialogVisible" title="新增商品" width="560px" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="createFormRef" :model="createForm" :rules="createRules" label-width="90px">
        <el-row :gutter="16">
          <el-col :span="14">
            <el-form-item label="商品名称" prop="name"><el-input v-model="createForm.name" maxlength="100" /></el-form-item>
          </el-col>
          <el-col :span="10">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="8">
            <el-form-item label="成本价 (¥)">
              <el-input-number v-model="createForm.cost_price" :min="0" :precision="2" style="width:100%" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-form-item label="关联库存">
          <el-input v-model="createForm.inventory_item_id" placeholder="库存物品ID（可选，用品类可关联）" />
        </el-form-item>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="editDialogVisible" title="编辑商品" width="560px" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="editFormRef" :model="editForm" label-width="90px">
        <el-row :gutter="16">
          <el-col :span="14">
            <el-form-item label="商品名称"><el-input v-model="editForm.name" maxlength="100" /></el-form-item>
          </el-col>
          <el-col :span="10">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="8">
            <el-form-item label="成本价 (¥)">
              <el-input-number v-model="editForm.cost_price" :min="0" :precision="2" style="width:100%" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="库存数量">
